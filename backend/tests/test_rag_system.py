...
class TestRAGSystem:
    ...
    def test_add_course_document_success(self, test_rag_system, sample_course, sample_course_chunks):
        """Test successfully adding a course document"""
        # Mock document processor to return sample data
        with patch.object(test_rag_system.document_processor, 'process_course_document') as mock_process:
    ...
    def test_add_course_folder_success(self, test_rag_system, sample_course, sample_course_chunks):
    ...
    def test_add_course_folder_skip_existing(self, test_rag_system, sample_course, sample_course_chunks):
    ...
    def test_query_basic_functionality(self, mock_anthropic, test_rag_system, sample_course, sample_course_chunks):
    ...
    @patch('rag_system.anthropic.Anthropic')
    def test_query_with_tool_use(self, mock_anthropic, test_rag_system, sample_course, sample_course_chunks):
    ...
    def test_query_with_session_management(self, test_rag_system):
        ...
        with patch.object(test_rag_system.ai_generator, 'generate_response') as mock_generate:
    ...
    def test_source_management(self, test_rag_system, sample_course, sample_course_chunks):
        ...
        with patch.object(test_rag_system.ai_generator, 'generate_response') as mock_generate:
            def mock_ai_with_tool_use(*args, **kwargs):
                ...
                if 'tool_manager' in kwargs:
                    test_rag_system.tool_manager.execute_tool('search_course_content', query='machine learning')
        ...
        with patch.object(test_rag_system.ai_generator, 'generate_response') as mock_generate:
            def mock_ai_with_empty_search(*args, **kwargs):
                if 'tool_manager' in kwargs:
                    ...
                    result = test_rag_system.tool_manager.execute_tool('search_course_content', query='anything')
    ...
    def test_different_query_types(self, test_rag_system, sample_course, sample_course_chunks, query_type, expected_tool):
            ...
            def mock_ai_selective_tool(*args, **kwargs):
                if 'tool_manager' in kwargs:
                    if expected_tool == "get_course_outline":
                        return test_rag_system.tool_manager.execute_tool('get_course_outline', course_name='Machine Learning')
                    else:
                        return test_rag_system.tool_manager.execute_tool('search_course_content', query=query_type)
    ...
    def test_concurrent_sessions(self, test_rag_system):
        ...
        with patch.object(test_rag_system.ai_generator, 'generate_response') as mock_generate:
            ...
            history1 = test_rag_system.session_manager.get_conversation_history(session1)
            history2 = test_rag_system.session_manager.get_conversation_history(session2)
